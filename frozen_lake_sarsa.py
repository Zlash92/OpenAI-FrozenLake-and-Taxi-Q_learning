...
def q_frozen_lake(gamma=0.99, alpha=0.1, epsilon=0.01):
    """
    ...
    for episode_count in range(number_of_episodes):
        ...
        if random.random() < epsilon:
            ...
            next_action = pick_random_action_among_max(q[state])
        ...
        for t in range(iterations_per_episode):
            ...
            # Update step
            q[pre_move_state][last_action] += alpha * (reward + gamma * q[state][next_action] - q[pre_move_state][last_action])
    ...
    print("Average reward per episode:", float(total_time_won) / number_of_episodes)
